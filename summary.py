...
def parse_args():
    ...
    parser.add_argument("--model_dir", "-d", default="../prepare_datasets/data/data_edf_20_npz/EEGEOG", help="where the data is.")
    parser.add_argument("--modal", '-m', default='0',
                        help="the way to training model\n\t0: single modal.\n\tmulti modal.")
    parser.add_argument("--output_dir", '-o', default='./result3', help="where you wish to set the results.")
...
def plot(true_stages,pred_stages,i,path):
    # plot result
    true_stages = true_stages[500:1000]
    pred_stages = pred_stages[500:1000]
    plt.figure(figsize=(16,8))
    plt.xlim((0, len(true_stages)))
    plt.ylim((0, 6))
    plt.plot(true_stages + 1, color='crimson', label="True Label")
    plt.plot(pred_stages + 1, color='royalblue', label="Predict")
...
def summary_models(args: argparse.Namespace, hyper_params: dict):
    ...
    eva_model: tensorflow.keras.models.Model = U2Model(**hyper_params)
    ...
    for i in range(k_folds):
        ...
        plot(test_labels.argmax(axis=1), y_pred.argmax(axis=1), i, path=res_dir)
